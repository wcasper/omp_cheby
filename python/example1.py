...
# Print out results
##
# We print out the results in pretty matrix form, using sympy to express their
# values in terms of closest rationals
##

print("Printing the B sequence:")
for k in range(degmax):
  print("B%i" % k)
  sympy.pprint(sympy.nsimplify(sympy.Matrix(B[k,:,:]),tolerance=1e-10,rational=True))
  
print("Printing the C sequence:")
for k in range(1,degmax):
  print("C%i" % k)
  sympy.pprint(sympy.nsimplify(sympy.Matrix(C[k,:,:]),tolerance=1e-10,rational=True))
